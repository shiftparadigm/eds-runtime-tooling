...
export const entryGlobs = ['src/scripts/*', 'src/blocks/*/*'];
...
function buildEntry() {
	...
	function isValidBlockFile(path: string) {
		const match = /^src\/blocks\/(?<folder>.*)\/(?<file>.*)\.m?(t|j)s/g.exec(
			path,
		);
...
const EntryWatcher = (): PluginOption => {
	...
	function writeLog(message: string) {
		viteConfig.logger.info(colors.green('[Edge-Delivery-Tools] ') + message);
	}
	...
	return {
		...
		config(config) {
			...
			if (initialEntry === undefined) {
				if (typeof lib.entry !== 'object' || Array.isArray(lib.entry))
					throw new Error('build.lib.entry must be an object');
		...
		configResolved(config) {
			...
			if (keys.length)
				writeLog(
					colors.dim('Using overridden entrypoints: ') +
